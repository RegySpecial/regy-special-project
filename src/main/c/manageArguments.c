...
gameContextStructure manageHelpFlag(int argc, char *argv[])
{
  ...
  const char
    *const trueHelp[] = {
      "-help [user=name][,user=identifier][,debug=assertions][,debug=shell][,background=color][,background=pixelmap][,background=path][,audio=path][,audio=beepmap][,audio=volume]"
      ...
      case argumentOptionHelpFlagSuboptions_user:
        if (suboptionValue)
          for (unsigned char index = 0; index < sizeof helpSuboptions / sizeof *helpSuboptions; index++)
            if (!strcmp(suboptionValue, userSuboptions[index]))
            {
              char* const suboptionCopy = userSuboptions[index];
      ...
      case argumentOptionBackgroundFlagSuboptions_color:
        ...
        if (!(regularExpressionExecution = regexec(&regularExpression, suboptionValue, 1, &occurrency, REG_ICASE | REG_EXTENDED)))
        {
          ...
          if (!strcmp(colorFuntionString,"hsl"))
          {
            ...
            for (char* paramGet = strtok(colorFuntionString + 4, ","); paramGet; paramGet = strtok(NULL, ","), index++)
          ...
          else if (!strcmp(colorFuntionString,"hsla"))
          {
            hsla HSLA={};
            unsigned char index = 0;
            for (char* paramGet = strtok(colorFuntionString + 5, ","); paramGet; paramGet = strtok(NULL, ","), index++)
          ...
          else if (!strcmp(colorFuntionString,"cmyka"))
          {
            cmyka CMYKA={};
            unsigned char index = 0;
            for (char* paramGet = strtok(colorFuntionString + 6, ","); paramGet; paramGet = strtok(NULL, ","), index++)
